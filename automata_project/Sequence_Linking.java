...
public class Sequence_Linking {
	...
	/*
	 * This function exists because in given files we have names of states like a, b and values, and we have to put the values
	 * into array which (as we are in Java) must be indexed numerically, so we can't do array[b] = value.
	 * And I haven't figured out other way to do it.
	 */
	private static int checkState(char ch){
		switch (ch) {
		case 'a': return 0;
		case 'b': return 1;
		case 'c': return 2;
	...
	private static boolean matchSequenceWithInitialStates(int seqNumber, int[] initialStates){
		...
		for (int j = 0; j < lengths[seqNumber]; j++) {
			...
			for (int k = 0; k < 2; k++){
				...
				int a = checkState(sequences[seqNumber][j].charAt(positionInLine));
				if (x[a] == -1) {
	...
	private static boolean applySequence(int seqNumber, int[] initialStates){
		...
		for (int i = 0; i < lengths[seqNumber]; i++){
			...
			int stateChanger = checkState(sequences[seqNumber][i].charAt(0));
			...
			int stateToChange = checkState(sequences[seqNumber][i].charAt(7));			
	 ...
	 * we can use applySequence for that one and then matchSequenceWithInitialStates for those hundred and current
	 * sequence set by applySequence.
	 */
